...
export function FixedVerticalProcessChain({ activities, onActivityClick, className }: FixedVerticalProcessChainProps) {
  ...
  if (chainActivities.length === 0) {
    return (
      <div className={cn("fixed left-4 top-1/2 transform -translate-y-1/2 z-40 flex flex-col items-center justify-center py-4 text-muted-foreground", className)}>
  ...
  return (
    <div className={cn("fixed left-4 top-1/2 transform -translate-y-1/2 z-40", className)}>
      {/* Main chain container */}
      <div className="relative">
        {/* Background indicator */}
        {hasActiveProcess && (
          <div className="absolute -left-2 -right-2 -top-4 -bottom-4 bg-muted/5 rounded-full animate-pulse" />
        )}
      ...
      {showTooltip && hoveredActivity && (
        <div className="absolute left-8 top-1/2 transform -translate-y-1/2 bg-popover/95 backdrop-blur-sm text-popover-foreground px-3 py-2 rounded-md shadow-xl border border-border pointer-events-none transition-all duration-200 whitespace-nowrap z-50 max-w-xs">
