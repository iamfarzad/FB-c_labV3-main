...
const createMockSupabaseClient = () => {
  // Using mock Supabase client - environment variables missing
...
export const getSupabase = () => {
  ...
  if (!config.supabase.url || !config.supabase.anonKey) {
    if (isDevelopment || enableMocking) {
      // Development mode: Using mock Supabase client due to missing environment variables
      return createMockSupabaseClient()
    } else {
              // Supabase environment variables are missing. Please check SUPABASE_URL and SUPABASE_ANON_KEY.
      return null
    }
  }
...
export const supabase = (() => {
  if (!supabaseInstance) {
    ...
    if (!supabaseInstance) {
      // Fallback to mock Supabase client due to missing environment variables
